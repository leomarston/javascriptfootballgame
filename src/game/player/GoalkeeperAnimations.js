...
function jumpClip() {
  const D = 0.85;
  const T = [0, 0.3, 0.5, 0.85];
  return clip('jump', D, [
    p('hips', T, [[0, HIP_Y - 0.12, 0], [0, HIP_Y + 0.04, 0], [0, HIP_Y + 0.08, 0], [0, HIP_Y - 0.04, 0]]),
    k('hips', [0, 0, 0]),
    q('spine', T, [[0.2, 0, 0], [0.05, 0, 0], [0.02, 0, 0], [0.18, 0, 0]]),
    k('chest', [0.05, 0, 0]),
    q('head', T, [[-0.1, 0, 0], [-0.2, 0, 0], [-0.22, 0, 0], [-0.12, 0, 0]]),
    // arms thrust overhead
    q('upperArmL', T, [[0.2, 0, 0.4], [-2.3, 0, 0.2], [-2.6, 0, 0.15], [-0.9, 0, 0.5]]),
    q('upperArmR', T, [[0.2, 0, -0.4], [-2.3, 0, -0.2], [-2.6, 0, -0.15], [-0.9, 0, -0.5]]),
    q('lowerArmL', T, [[-0.5, 0, 0], [-0.15, 0, 0], [-0.1, 0, 0], [-0.4, 0, 0]]),
    q('lowerArmR', T, [[-0.5, 0, 0], [-0.15, 0, 0], [-0.1, 0, 0], [-0.4, 0, 0]]),
    // crouch then tuck the knees up
    q('thighL', T, [[0.5, 0, 0.14], [-0.05, 0, 0.1], [0.55, 0, 0.12], [0.45, 0, 0.14]]),
    q('thighR', T, [[0.5, 0, -0.14], [-0.05, 0, -0.1], [0.55, 0, -0.12], [0.45, 0, -0.14]]),
    q('shinL', T, [[0.9, 0, 0], [0.3, 0, 0], [1.0, 0, 0], [0.8, 0, 0]]),
    q('shinR', T, [[0.9, 0, 0], [0.3, 0, 0], [1.0, 0, 0], [0.8, 0, 0]]),
    k('footL', [0.1, 0, 0]),
    k('footR', [0.1, 0, 0])
  ]);
}
